...
object OverlayHelper {
    ...
    fun showFrictionOverlay(context: Context, domain: String) {
        handler.post {
            if (overlayView != null) {
                FileLog.write(context, "OverlayHelper: overlay already showing, ignoring new request for $domain")
                return@post
            }
            if (!canShowOverlay(context)) {
                FileLog.write(context, "OverlayHelper: overlay permission not granted, cannot show for $domain")
                return@post
            }
            ...
            try {
                ...
                FileLog.write(appContext, "OverlayHelper: overlay shown for $domain")
    ...
    private fun removeOverlay(context: Context) {
        ...
        try {
            ...
            FileLog.write(context, "OverlayHelper: removeView failed: ${e.message}")
        }
        overlayView = null
        windowManager = null
        currentDomain = null
